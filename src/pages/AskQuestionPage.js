...
const AskQuestionPage = () => {
  ...
  const { register, handleSubmit, reset, errors } = useForm({
    defaultValues: {
      ...
      department: editValues ? editValues.department: '',
      courseName: editValues ? editValues.courseName: '',
      courseCode: editValues ? editValues.courseCode: '',
  ...
  const postQuestion = ({ title, department, courseName, courseCode, body }) => {
    if (tags.length === 0) return setErrorMsg('Atleast one tag must be added.');

    addQuestion({
      variables: { title, department, courseName, courseCode, body, tags },
      update: (_, { data }) => {
  ...
  const editQuestion = ({ title, department, courseName, courseCode, body }) => {
    if (tags.length === 0) return setErrorMsg('Atleast one tag must be added.');

    updateQuestion({
      variables: { quesId: editValues.quesId, title, department, courseName, courseCode, body, tags },
      update: (_, { data }) => {
      ...
      >
        ...
        <div className={classes.inputWrapper}>
          <TextField
            ...
            inputRef={register}
            name="department"
            placeholder="Enter the name of your department e.g computer science"
            type="text"
            label="Department"
            variant="outlined"
            size="small"
        ...
        <div className={classes.inputWrapper}>
          <TextField
            ...
            inputRef={register}
            name="courseName"
            placeholder="Enter the name of your course e.g intro to computer science"
            type="text"
            label="Course Name"
            variant="outlined"
            size="small"
            error={'courseName' in errors}
            helperText={'courseName' in errors ? errors.courseName.message : ''}
            className={classes.inputField}
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
        ...
        <div className={classes.inputWrapper}>
          <TextField
            ...
            inputRef={register}
            name="courseCode"
            placeholder="Enter your course code e.g CSC101"
            type="text"
            label="Course Code"
            variant="outlined"
            size="small"
            error={'courseCode' in errors}
            helperText={'courseCode' in errors ? errors.courseCode.message : ''}
            className={classes.inputField}
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
